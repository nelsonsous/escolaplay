...
const AVATAR_VAMPIRE = [
    'vampire:elena', 'vampire:damon', 'vampire:stefan', 'vampire:caroline',
...
const AVATAR_STRANGER = [
    ...
    'stranger:dustin', 'stranger:max',    'stranger:steve',   'stranger:robin',
    'stranger:nancy',  'stranger:jonathan','stranger:hopper', 'stranger:joyce',
    'stranger:billy',  'stranger:erica',  'stranger:murray',  'stranger:mayor'
];
...
const YEAR_EXTRA_FILES = {
    2: [
        ...
        { src: 'content_2_m_extra4.js', varName: 'EXERCISES_2_M_EXTRA4' },
    ...
    6: [
        ...
        { src: 'content_6_c_extra3.js', varName: 'EXERCISES_6_C_EXTRA3' },
        { src: 'content_6_c_extra4.js', varName: 'EXERCISES_6_C_EXTRA4' },
    ...
    const html = `
        <div class="fullscreen" id="subject-detail-screen">
            ...
            <div class="exercise-body">
                ...
                <!-- ===== BLOCO MAX ===== -->
                <div style="margin-top:16px;background:linear-gradient(135deg,#4c1d95,#f472b6);border-radius:16px;padding:14px;box-shadow:0 4px 16px rgba(109,40,217,0.35)">
                    ...
                    <div id="max-topic-sel-bar" style="display:none;background:rgba(255,255,255,0.12);border-radius:10px;padding:10px;margin-bottom:10px">
                        ...
                        <button class="btn btn-block" style="margin-bottom:6px;background:#fff;color:#f472b6;border-radius:10px;padding:10px;font-weight:700;font-size:0.88rem" onclick="startMaxForSelected('${key}', false)">
...
function _updateTopicFocusPill() {
...
function renderTopicList() {
    ...
    if (!container) return;
    const seen = state.exerciseSeen || {};
    const subColor = SUBJECTS[key]?.color || '#f472b6';
            ...
            <div class="modal-content" style="max-width:560px;max-height:90vh;border-radius:18px;display:flex;flex-direction:column">
                ...
                <div class="modal-body" style="flex:1;overflow-y:auto">
                    <div style="display:flex;gap:8px;margin-bottom:14px;font-size:0.78rem;flex-wrap:wrap">
                        <span style="background:#fce7f3;color:#f472b6;padding:4px 10px;border-radius:999px;font-weight:700">📖 ${pool.length} no total</span>
        ...
        <div id="all-answered-modal-temp" class="modal" style="display:flex;align-items:center;justify-content:center;padding:20px">
            <div class="modal-content" style="max-width:480px;border-radius:18px;max-height:90vh">
                ...
                <div class="modal-body" style="text-align:center">
                    ...
                    <button class="btn btn-block" onclick="closeAllAnsweredModal();startMaxForTest('${testId}')" style="background:linear-gradient(135deg,#f472b6,#f472b6);color:#fff;border:none;font-weight:700;padding:14px;margin-bottom:10px">
        ...
        <div id="all-answered-subject-modal-temp" class="modal" style="display:flex;align-items:center;justify-content:center;padding:20px">
            <div class="modal-content" style="max-width:480px;border-radius:18px;max-height:90vh">
                ...
                <div class="modal-body" style="text-align:center">
                    ...
                    <button class="btn btn-block" onclick="generateMaxForSubjectAnswered()" style="background:linear-gradient(135deg,#f472b6,#f472b6);color:#fff;border:none;font-weight:700;padding:14px;margin-bottom:10px">
                        <i class="fas fa-wand-magic-sparkles"></i> Carregar novas com IA MAX
                    </button>
...
[{"q":"English question","options":["English","English","English"],"answer":0,"exp":"English 1 line","expPt":"nota PT-PT curta","topic":"PT-PT category"}]`;
    try {
        ...
        if (!tutorState) return;
        const items = _tutorParseExercises(text).slice(0, 3);
...
function _tutorParseExercises(text) {
    ...
    return (items || []).filter(it => it && it.q && Array.isArray(it.options) && it.options.length >= 2 && typeof it.answer === 'number');
}
...
function _tutorPracticeAnswer(i) {
    ...
    // Após responder, lê em voz alta a frase completa já com a palavra certa
    // (ou só a palavra, se a pergunta não tiver lacuna) — antes a lacuna era lida como "blank".
    const _correctOpt = it.options[it.answer] || '';
    const _sayCorrect = (/_{2,}/.test(it.q) ? it.q.replace(/_{2,}/, _correctOpt) : _correctOpt).replace(/\s+/g, ' ').trim();
    if (_sayCorrect && typeof speakEN === 'function') _tutorSpeak(_sayCorrect);
    _tutorScroll();
    pq.queue.shift(); // remove o exercício respondido
    if (ok) {
        setTimeout(() => { if (tutorState && tutorState._pq) _tutorRenderPracticeItem(); }, 700);
    } else {
        setTimeout(() => { if (tutorState && tutorState._pq) _tutorPracticeMistake(it, i); }, 500);
    }
}
...
// Gera (via Mistral/Groq) uma micro-lição + 3 exercícios sobre o erro concreto
...
function _launchConfetti(intensity) {
    ...
    const colors = ['#f59e0b','#ef4444','#f472b6','#f472b6','#3b82f6','#10b981','#facc15'];
...
function _showDuelSummary(s) {
    ...
    else if (tied){ title = 'Empate!';         emoji = '🤝'; gradient = 'linear-gradient(135deg,#f472b6 0%,#f472b6 50%,#06b6d4 100%)'; }
            ...
            <div class="modal-body" style="padding:22px">
                ...
                <div style="display:grid;grid-template-columns:1fr auto 1fr;gap:14px;align-items:stretch;margin-bottom:18px">
                    <div style="background:${won ? '#fef3c7' : tied ? '#fce7f3' : '#fff'};border:2px solid ${won ? '#f59e0b' : tied ? '#f9a8d4' : 'var(--border)'};border-radius:18px;padding:16px 12px;text-align:center">
                        ...
                        <div style="font-size:1.7rem;font-weight:900;color:${won ? '#d97706' : tied ? '#f472b6' : 'var(--text-light)'};letter-spacing:-0.02em">${myScore}</div>
...
function _renderHintHtml(rawText) {
    ...
    if (conceitoMatch) {
        ...
        html += `<div style="margin-bottom:12px"><div style="font-weight:700;color:#f472b6;margin-bottom:4px;font-size:0.85rem">📚 Conceito</div><div style="color:#9d174d">${body}</div></div>`;
    }
    if (dicasMatch) {
        ...
        if (bullets.length >= 2) {
            dicasHtml = '<ul style="margin:0;padding-left:20px;list-style:none">' +
                bullets.map(l => `<li style="margin-bottom:6px;position:relative;padding-left:4px"><span style="position:absolute;left:-16px;color:#f472b6">•</span>${escapeHtml(l.replace(/^[•\-*]\s*/, ''))}</li>`).join('') +
                '</ul>';
        } else {
            dicasHtml = `<div>${escapeHtml(dicas).replace(/\n+/g, '<br>')}</div>`;
        }
        html += `<div style="margin-bottom:12px"><div style="font-weight:700;color:#f472b6;margin-bottom:6px;font-size:0.85rem">💡 Como pensar</div><div style="color:#9d174d">${dicasHtml}</div></div>`;
    }
    if (encMatch) {
        html += `<div style="margin-top:10px;padding-top:8px;border-top:1px dashed #fbcfe8;color:#f472b6;font-style:italic;font-size:0.88rem">✨ ${escapeHtml(encMatch[1].trim())}</div>`;
    }
...
async function _loadAndShowHint(textEl, btnEl, boxEl) {
    ...
    textEl.innerHTML = '<div style="color:#f472b6">A preparar a pista…</div>';
    boxEl.style.display = 'block';
...
function _hintAiBox(text) {
    return `<div style="background:linear-gradient(135deg,#fdf2f8,#fce7f3);border-left:4px solid #f472b6;border-radius:10px;padding:14px 16px;margin-top:4px">
        <div style="font-size:0.78rem;font-weight:700;color:#f472b6;margin-bottom:8px;letter-spacing:.03em">🎓 PROFESSOR IA</div>
        <div style="font-size:0.9rem;line-height:1.6;color:#9d174d">${_renderHintHtml(text)}</div>
    </div>`;
}
...
function renderSecretModalList() {
    ...
    list.innerHTML = visible.map(p => {
        ...
        const btn = isAdded
            ...
            : `<button onclick="addSecretPack('${p.id}')" style="background:linear-gradient(135deg,#f472b6,#db2777);color:#fff;border:none;border-radius:8px;padding:8px 14px;font-size:0.84rem;font-weight:700;cursor:pointer">+ Adicionar</button>`;
        return `
...
function _teacherStartRead(resume) {
    ...
    r.onresult = (event) => {
        ...
        // Transcrito ao vivo: últimas 8 palavras.
        ...
        // Auto-deteção: se as últimas 2 palavras do parágrafo aparecem no
